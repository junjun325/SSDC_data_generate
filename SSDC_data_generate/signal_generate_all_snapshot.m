function [X,s,a,am_vec]=signal_generate_all_snapshot(M,snapshot,DOA,f0,fc,fs,flag,k)
% % M 阵元数
% % snapshot 快拍数
% % DOA 到达角
% %fc 信号频率
...
%%  利用噪声源
if flag==1
    am_vec=2*rand(1,snapshot(k))-1;
elseif flag==2
    am_vec=randn(1,snapshot(k));
elseif flag==3
    am_vec=ones(1,snapshot(k));
end
a=exp(1i*pi*fc*(0:M-1).'*sind(DOA')/f0);%;
s=randn(2,snapshot(k))+sqrt(-1)*randn(2,snapshot(k));
S(1,:) = s(1,:);
S(2,:) = s(1,:);
